...
echo  use Ansible to ping localhost in /vagrant/home/inventory
...
echo  add remote host keys to known hosts
ansible 127.0.0.1 -i /vagrant/ansible/inventory -m shell -a "ssh-keyscan 192.168.33.11 >> ~/.ssh/known_hosts" -u vagrant -k
ansible 127.0.0.1 -i /vagrant/ansible/inventory -m shell -a "ssh-keyscan 192.168.33.20 >> ~/.ssh/known_hosts" -u vagrant -k
ansible 127.0.0.1 -i /vagrant/ansible/inventory -m shell -a "ssh-keyscan 192.168.33.30 >> ~/.ssh/known_hosts" -u vagrant -k
